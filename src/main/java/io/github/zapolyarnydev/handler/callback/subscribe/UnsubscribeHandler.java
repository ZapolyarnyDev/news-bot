...
public final class UnsubscribeHandler extends CallbackHandler {
    ...
    public UnsubscribeHandler() {
        super(List.of("main_unsubscribe"));
    }
    ...
    public List<TelegramAction> handle(Update update) {
        ...
        if(subscriptionService.isSubscribed(chatId)){
            editMessage.setText(messageService.getMessage("use-unsubscribe"));
            subscriptionService.unsubscribe(chatId);
        } else {
            editMessage.setText(messageService.getMessage("already-unsubscribed"));
        }
